document.addEventListener('DOMContentLoaded', function() {
    ...
    // Start handling the loading screen with optimized timing
    handleLoadingScreen(2000);
});

// Window onload ensures all resources (images, etc.) are fully loaded
window.addEventListener('load', function() {
    // Ensure minimum 2 seconds for smooth experience
    handleLoadingScreen(2000);
});

// Separate function to handle the loading screen with configurable timing
function handleLoadingScreen(minTime = 2000) {
    try {
        ...
        // Only add the fade-out if it's not already added
        if (!loadingScreen.classList.contains('fade-out')) {
            // Give the spinner optimized time to show (2 seconds for better UX)
            setTimeout(function() {
                // Add fade-out class to trigger the CSS transition
                loadingScreen.classList.add('fade-out');
                
                // Remove from DOM after transition completes
                loadingScreen.addEventListener('transitionend', function onTransitionEnd(e) {
                    // Only proceed if the transition is for opacity
                ...
                }, 600); // Fallback after 0.6 seconds (transition is 0.5s)
            }, minTime);
        }
    } catch (error) {
        console.error('Error handling loading screen:', error);
    }
}
...
function initFilterFunctionality() {
    ...
    const filterCards = async (filter) => {
        ...
        elements.emptyState.classList.toggle('visible', !hasVisibleCards);
            ...
            const filterName = filter === 'all' ? 'all content' : filter + 's';
